...
int debug = 1;
// max_serial_number is used in debugging. It causes the app
// to stop after a specified number of phones are unwrapped.
// It's approximate as I didn't bother using to mutex to
// compare to current_serial_number. Set to -1 for normal 
// behavior.
int max_serial_number = 20;
...
int unpack(int *arg) {
	...
	while(1) {
		...
		if (debug) printf("Phone unwrapped. Serial number = %i\n", new_serial_number); //TODO: remove this line
		...
		if (debug && current_serial_number >= max_serial_number) minithread_stop();
